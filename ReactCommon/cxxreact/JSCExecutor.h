...
public:
  JSCExecutorFactory(
      ...
      std::function<folly::dynamic(const std::string &)> provider)
...
template <typename T> struct JSCValueEncoder {
  // If you get a build error here, it means the compiler can't see the template
  // instantation of toJSCValue
  // applicable to your type.
  static const Value toJSCValue(JSGlobalContextRef ctx, T &&value);
};

template <> struct JSCValueEncoder<folly::dynamic> {
  static const Value toJSCValue(JSGlobalContextRef ctx,
...
public:
  ...
  explicit JSCExecutor(std::shared_ptr<ExecutorDelegate> delegate,
                       ...
                       std::function<folly::dynamic(const std::string &)>
                           nativeExtensionsProvider) throw(JSException);
  ...
  virtual void
  setBundleRegistry(std::unique_ptr<RAMBundleRegistry> bundleRegistry) override;
  virtual void registerBundle(uint32_t bundleId,
                              const std::string &bundlePath) override;

  virtual void callFunction(const std::string &moduleId,
                            const std::string &methodId,
                            const folly::dynamic &arguments) override;

  virtual void invokeCallback(const double callbackId,
                              const folly::dynamic &arguments) override;

  virtual void
  setGlobalVariable(std::string propName,
...
private:
  ...
  static bool isNetworkInspected(const std::string &owner,
  ...
  JSValueRef nativeFlushQueueImmediate(size_t argumentCount,
                                       const JSValueRef arguments[]);
  JSValueRef nativeCallSyncHook(size_t argumentCount,
                                const JSValueRef arguments[]);
};
}
}
